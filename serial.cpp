...
Serial::Serial(const unsigned int BandRate){
    this->BandRate=BandRate;
    this->Bufsize=SERIAL_BUF_SIZE;
}

bool Serial::init(const std::string com){
    bool ret;
...
bool Serial::send(const char data){
    DWORD dwSendSize;
    COMSTAT Comstat;
    DWORD dwErrorMask;
...
bool Serial::send(const char *data,const unsigned int size){
...
bool Serial::send(const std::string str){
...
unsigned long Serial::available(){
    DWORD dwErrors;
    COMSTAT ComStat;
    DWORD dwCount;
